...
public class MyResources {
    ...
    MyResources(FileSpec file, String packageName, Resources res, AssetManager asset, MyResources[] deps){
        this.file = file;
    ...
    static MyResources getResource(MyClassLoader mcl) {
        ...
        File dir = MyApplication.instance().getFilesDir();
        dir = new File(dir, "repo");
        if (!dir.isDirectory()) throw new RuntimeException(dir + " not exists");
        dir = new File(dir, file.id());
        File path = new File(dir, TextUtils.isEmpty(file.md5()) ? "1.apk" : file.md5() + ".apk");
        if (!path.isFile()) throw new RuntimeException(path + " not exists");
